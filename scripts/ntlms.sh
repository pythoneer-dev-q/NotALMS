#!/usr/bin/env bash
# ! scripts/ntlms.sh — алиас ntlms: up/down/restart/status/logs/edit-env/edit/update
...
# docker-команды
d_up()      { (cd "$NTLMS_ROOT" && docker compose up -d); }
d_down()    { (cd "$NTLMS_ROOT" && docker compose down); }
d_restart() { (cd "$NTLMS_ROOT" && docker compose restart "$@"); }
d_status()  { (cd "$NTLMS_ROOT" && docker compose ps); }
d_logs()    { (cd "$NTLMS_ROOT" && docker compose logs -f --tail=100 "$@"); }
